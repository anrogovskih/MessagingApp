...
/**
 * Original view taken from
 * https://github.com/leochuan/BubbleView
 *
 * Реализацию этой ViewGroup с тенью можно посмотреть в приложении заключенного
 */
public class BubbleLayout extends ViewGroup {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        orientation = LEFT;
        borderWidth = Utils.INSTANCE.dip2px(context, 2);
        radius = Utils.INSTANCE.dip2px(context, 5);
        triangleWidth = Utils.INSTANCE.dip2px(context, 12);
        triangleHeight = Utils.INSTANCE.dip2px(context, 8);
        offset = Utils.INSTANCE.dip2px(context, 8);

        if (attrs == null) return;
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.BubbleLayout);
        backgroundColor = typedArray.getColor(R.styleable.BubbleLayout_bgColor, backgroundColor);
        borderColor = typedArray.getColor(R.styleable.BubbleLayout_borderColor, borderColor);
        offset = (int) typedArray.getDimension(R.styleable.BubbleLayout_offset, offset);
        borderWidth = (int) typedArray.getDimension(R.styleable.BubbleLayout_borderWidth, borderWidth);
        radius = (int) typedArray.getDimension(R.styleable.BubbleLayout_radius, radius);
        orientation = typedArray.getInt(R.styleable.BubbleLayout_orientation, orientation);
        triangleWidth = (int) typedArray.getDimension(R.styleable.BubbleLayout_triangleWidth, triangleWidth);
        triangleHeight = (int) typedArray.getDimension(R.styleable.BubbleLayout_triangleHeight, triangleHeight);
        clipToRadius = typedArray.getBoolean(R.styleable.BubbleLayout_clipToRadius, clipToRadius);
        centerArrow = typedArray.getBoolean(R.styleable.BubbleLayout_centerArrow, centerArrow);
        typedArray.recycle();
    }
    ...
    private void generatorLefPath() {
        path.moveTo(getBorderLeft(), getBorderBottom());
        path.lineTo(getBorderLeft() + triangleHeight, getBorderBottom() - triangleWidth);
    ...
    private void generatorRightPath() {
        path.moveTo(getBorderRight(), getBorderBottom());
        path.lineTo(getBorderRight() - triangleHeight, getBorderBottom() - triangleWidth);
    ...
    private float getBorderBottomForShadow() {
